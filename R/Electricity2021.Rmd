...
Electricity %<>%
  data.frame(row.names = c(Province, "계"))
Electricity %<>%
 `/`(1000) %>%
  round 
Electricity[, "계", drop = FALSE]
Electricity
```
...
m_list <- mosaic_gg(tbl = Electricity_Sales)
m <- m_list$m
df <- m_list$df
p_df <- m_list$p_df
str(p_df)
...
pander(data.frame(A, W_A, B, W_B, SS),
       col.names = c("발전량", "발전량비중", "판매량", "판매량비중", "전력자급도(%)"))
```
...
Electricity <- 
  data.frame(Province, R, R2, A, B, SS)
pander(Electricity, col.names = c("시도", "5극3특", "5대권역", "발전량", "판매량", "전력자급도(%)"))
Electricity$R2 <- factor(Electricity$R2, 
                         levels = c("수도권", "가야", "백제", "신라", "영산강"))
Electricity %>%
  pander(col.names = c("시도", "5극3특", "5대권역", "발전량", "판매량", "전력자급도(%)"))
