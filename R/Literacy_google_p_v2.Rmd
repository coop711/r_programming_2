...
literacy_tbl2 <- literacy_tbl %>%
  ...
  mutate(Item = factor(Item, levels = 1:4))
ggplot(data = literacy_tbl2, mapping = aes(x = Questions, y = Counts, fill = Item)) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
  geom_text(aes(y = c(x_text)), label = c(label_ptbl), position = "identity") +
  scale_fill_brewer(name = "답문항", type = "qual", palette = "Accent") +
  xlab("문제") +
  ylab("백분율") +
  theme_minimal(base_family = "HCR Dotum LVT") +
  ...
  geom_text(aes(x = literacy_tbl2$Questions, y = -3), label = literacy_tbl2$Questions) +
  coord_flip()
```
