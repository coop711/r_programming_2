...
>>>>>>> 4170b3a6a6bb6e898dbb0c2eb31ed20270ba1496
...
표준오차를 계산해 보면 $1/sqrt(N)$ 범위 안에 들어감을 확인할 필요. $N = $`N`일 때, 표준오차는 `r 1/sqrt(N)` 임
...
매칭의 개수와 빈도를 테이블로 정리하고, `r N / 24` 로 나누어 이론적 확률 확인.
...
## Chisquare Test : simulated p-value
...
#> 만약 관찰된 카이제곱 통계량이 22였다면, simulated p_value 는
<<<<<<< HEAD
p_value <- sum(chi_squares >= 18) / (B + 1)
=======
p_value <- sum(chi_squares >= 22) / (B + 1)
>>>>>>> 4170b3a6a6bb6e898dbb0c2eb31ed20270ba1496
p_value
```
