---
title: "Literacy R Programming 2 : 2102"
author: "coop711"
date: "2021-10-26"
...
# literacy_roll <- 
#   read.xlsx("../data/literacy_1902.xlsx", 
#             sheetIndex = 1, 
#             startRow = 1, 
#             endRow  = 69, 
#             colIndex = 2:27, 
...
#             encoding = "UTF-8")
# literacy_roll <-
#   openxlsx::read.xlsx("../data/literacy_1902.xlsx",
#                       sheet = 1,    
#                       colNames = TRUE,
#                       cols = 2:27,
#                       rows = 1:69)
literacy_roll <- 
   read_excel("../data/literacy_2102.xlsx", 
              range = "C1:AD64",
              col_names = TRUE,
              col_types = "text")
literacy_roll <- literacy_roll[, -c(2:3)]
...
#             xpd = TRUE, 
             ...
             args.legend = list(x = 75, y = 25, title = "답문항"))
# max(b)
# literacy_tbl[1, ]
# pos <- function(x){
#   cumsum(x) - x / 2
...
pos <- {. %>% {cumsum(.) - `/`(., 2)}}
x_text <- apply(literacy_tbl, 
                MARGIN = 2, 
                FUN = pos)
text(x = x_text, 
     y = rep(b, each = 4), 
     labels = label_ptbl)
title(main = "문해력조사 문항별 응답", 
      cex.main = 1.8, 
      line = -0.5)
```
...
ggplot(data = literacy_tbl2, 
       mapping = aes(x = Questions)) +
...
#> 퍼센티지를 추가 정보로 각 막대의 중점에 표시. mapping = aes() 에서 공통되는 x 좌표 설정은 생략 가능.
  geom_text(aes(y = x_text, 
                label = label_ptbl)) +
#> fill 의 색깔과 레전드 제목 설정.
  scale_fill_brewer(name = "답문항", 
                    type = "qual", 
                    palette = "Accent") +
#> 제목과 x축 라벨, y축 라벨 설정.
  labs(title = "문해력조사 문항별 응답", 
       x = "문제", 
       y = "백분율") +
#  xlab("문제") +
#  ylab("백분율") +
...
#> x축의 라벨을 막대에 보다 가깝게 조정 (flip해서 x축을 수직으로 세울 예정이므로 y = -3)
...
rate_item_type <- data.frame("유형" = type_kr, 
                             "정답률" = rate_item)
dimnames(rate_item_type) <- 
  list("문항" = paste0("문", 1:25), 
       "유형과 정답률" = c("유형", "정답률(%)"))
rate_item_type %>%
