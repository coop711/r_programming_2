...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(tidyverse) }\CommentTok{# `tidyverse`를 검색 경로에 올려 놓음. 함께 불러들이는 패키지들과 경고문에 유의.}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{VADeaths_tbl <-}\StringTok{ }\NormalTok{VADeaths }\OperatorTok{%>%}\StringTok{ }\CommentTok{# 최종 결과물을 `tibble` 형식으로 지정.}
\StringTok{  }\KeywordTok{as_tibble}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }\CommentTok{# 행렬 구조를 `tibble`구조로 변환. `tbl_df()`는 더 이상 사용되지 않음. }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Age =} \KeywordTok{row.names}\NormalTok{(VADeaths)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# 행 이름으로 주어진 연령대를 글자벡터로 생성 }
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{key =}\NormalTok{ Place_Gender, }\CommentTok{# `Age`를 제외한 나머지 변수를 `key, value`쌍으로 정리하면서 새로운 변수명 부여.}
         \DataTypeTok{value =}\NormalTok{ Rates, }
         \OperatorTok{-}\NormalTok{Age) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{separate}\NormalTok{(Place_Gender, }\KeywordTok{c}\NormalTok{(}\StringTok{"Place"}\NormalTok{, }\StringTok{"Gender"}\NormalTok{), }\CommentTok{# `Place_Gender`를 `Place`와 `Gender`로 분리.}
           \DataTypeTok{sep =} \StringTok{" "}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Age =} \KeywordTok{ordered}\NormalTok{(Age), }\CommentTok{# `Age`, `Place`, `Gender`를 순서형 범주와 명목형 범주로 변환}
         \DataTypeTok{Place =} \KeywordTok{factor}\NormalTok{(Place), }
         \DataTypeTok{Gender =} \KeywordTok{factor}\NormalTok{(Gender,  }\CommentTok{# `Gender`에서 `level = `를 설정하지 않으면 알파벳 순에 따라 수준이 정해짐.}
                         \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}\StringTok{"Male"}\NormalTok{, }\StringTok{"Female"}\NormalTok{))) }\CommentTok{# 즉, `Female`이 1, `Male`이 2가 됨.}
\NormalTok{VADeaths_tbl }\CommentTok{# `tibble` 형식으로 출력}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{VADeaths }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Age =} \KeywordTok{rownames}\NormalTok{(VADeaths))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{VADeaths }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{as_tibble}\NormalTok{() }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Age =} \KeywordTok{rownames}\NormalTok{(VADeaths)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{key =}\NormalTok{ Place_Gender, }
         \DataTypeTok{value =}\NormalTok{ Rates,}
...
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{key =}\NormalTok{ Place_Gender, }
         \DataTypeTok{value =}\NormalTok{ Rates,}
         \OperatorTok{-}\NormalTok{Age) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{separate}\NormalTok{(Place_Gender, }\KeywordTok{c}\NormalTok{(}\StringTok{"Place"}\NormalTok{, }\StringTok{"Gender"}\NormalTok{), }
...
\StringTok{  }\KeywordTok{separate}\NormalTok{(Place_Gender, }\KeywordTok{c}\NormalTok{(}\StringTok{"Place"}\NormalTok{, }\StringTok{"Gender"}\NormalTok{), }
           \DataTypeTok{sep =} \StringTok{" "}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Age =} \KeywordTok{ordered}\NormalTok{(Age),}
         \DataTypeTok{Place =} \KeywordTok{factor}\NormalTok{(Place),}
